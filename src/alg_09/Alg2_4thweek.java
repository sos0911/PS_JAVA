...
class Cal{
    ...
    int IsthereExtendedW(int x, int y, String word, boolean over){ // (5,C)에서 걸치는 word를 찾는다.
          int kindofC = ((y-1)*NofPart + x) % 5; // 글자의 종류
        int whatisthisC;
        int sum = 0;
        if(kindofC == 1 || kindofC == 4)
        ...
        visited[x][y] = true;
        for(int i = 0; i < 4; i++){
            boolean newover = over || (y + dy[i] < 1) || (y + dy[i] > 5);
            if(y + dy[i] < 1)
           sum += IsthereExtendedW(x + dx[i], y+5 + dy[i], word, newover);
            else{
                if(y + dy[i] > 5)
                sum += IsthereExtendedW(x + dx[i], y-5 + dy[i], word, newover);
                else{
                sum += IsthereExtendedW(x + dx[i], y + dy[i], word, newover);
                }
                 }
        }
        visited[x][y] = false;
...
public class Alg2_4thweek {
    ...
    public static void main(String[] args){
        ...
        for(int i = 1; i < NofPart + 1; i++)
            for(int j = 1; j < 6; j++){
                Rec5answer += c.Isthereword(i, j, word); // Rec5answer에는 (5,C) 직사각형 내에서 생성 가능한 word의 갯수를 저장
                 NofExtendedW += c.IsthereExtendedW(i, j, word, false);
            }
       ...
       // NofExtendedW = Rec10answer - Rec5answer*2;
            ...
            System.out.println(NofExtendedW);
        }
